...
from utility import test_set, experiments

np.random.seed(0)
EXPERIMENT = "../experiment_one"
TENSOR_PATH = "../TensorData"
RESULT_PATH = "Result"
DATA_PATH = "../preprocessing/step2_normalized/"

# Parameters of experiments
series = os.listdir(DATA_PATH)
temporal_sequence_considered = [30, 60, 100]
number_neurons_LSTM = [128, 256]
Testing_Set = test_set.get_testset("../testset_generated/from_2018_10_01_until_2019_01_15/test_set.txt")
features_to_exclude_from_scaling = ['stock_id', 'ChangePercentage', 'TradedQuantity',
                                    'TotalTradedVolumeIncludingBlocks']
...
for s in series:
    ...
    for temporal, neurons in product(temporal_sequence_considered, number_neurons_LSTM):
        dataset_tensor = experiments.fromtemporal_totensor(np.array(data_compliant), temporal,
                                                           ...
                                                           stock_name)
        # Dict for statistics
        predictions_file = {'stock_id': [], 'date': [], 'observed_norm': [], 'predicted_norm': [],
                            'observed_denorm': [],
                            'predicted_denorm': []}
        ...
        for data_tester in Testing_Set:
            ...
            x_train, y_train = train[:, :-1, :], train[:, -1, features_without_date.index('PriceOfLastTransaction')]
            x_test, y_test = test[:, :-1, :], test[:, -1, features_without_date.index('PriceOfLastTransaction')]
            ...
            predictions_file['stock_id'].append(stock_name)
            ...
            errors_file['stock_id'].append(stock_name)
