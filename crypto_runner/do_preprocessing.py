...
try:
    os.mkdir("../" + name_folder + "/" + folder_step_four)
except OSError:
    print("Creation of the directory %s failed" % folder_step_four)
else:
    print("Successfully created the directory %s " % folder_step_four)



for stock in os.listdir("../" + name_folder + "/" + folder_step_zero):
    after_data = False
    fileToRead=open("../"+name_folder+ "/" +folder_step_zero + "/"+ stock, "r")
    fileToWrite = open("../" + name_folder + "/" + folder_step_four + "/" +stock, "w")
    for line in fileToRead:
...
for stock in os.listdir("../" + name_folder + "/" + folder_step_four):
    n+=1

    fileToRead = open("../" + name_folder + "/" + folder_step_four + "/" + stock, "r")
    i = 0
    if primo:
        primo = False
        for line in fileToRead:
...
fileToWrite = open("../" + name_folder + "/" + folder_step_five + "/horizontal.csv", "w")
for line in file:
    fileToWrite.write(line)
fileToWrite.close()
