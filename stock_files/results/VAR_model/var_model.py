...
# Entry point
# dataset_path = 'data/var_model_data.csv'
dataset_path = 'horizontal-dataset.csv'
# stock_df = pd.read_csv(dataset_path, sep=',', header=0)
stock_df = pd.read_csv(dataset_path, sep=';', decimal=',', header=0)

stock_df['Date'] = stock_df['Date'].apply(lambda x: str_to_datetime(x))
...
excluded_features = ['stock_id', 'Date', 'ChangePercentage', 'TradedQuantity', 'TotalTradedVolumeIncludingBlocks']
# features = [f for f in features if f not in excluded_features]
features = [f for f in features if f.startswith('PriceOfLastTransaction')]
...
# dates_to_use = ['2017-02-03', '2017-05-23', '2017-09-18', '2017-01-19', '2016-08-09', '2017-07-03', '2016-12-01', '2016-09-29', '2016-10-26', '2016-03-01', '2016-02-26', '2016-08-17', '2017-01-18', '2016-10-10', '2017-08-08', '2016-04-26', '2016-10-21', '2016-05-17', '2016-02-09', '2017-03-06', '2017-03-31', '2017-04-13', '2017-05-08', '2016-02-18', '2016-05-12', '2016-02-23', '2016-10-13', '2016-06-09', '2016-10-17', '2017-02-13', '2016-06-07', '2017-05-04', '2017-07-21', '2017-07-20', '2016-03-21', '2016-01-07', '2016-12-08', '2017-10-12', '2016-12-22', '2017-07-06', '2017-02-28', '2016-04-07', '2016-07-21', '2016-11-08', '2017-08-24', '2016-09-19', '2017-10-23']
dates_to_use = get_test_dates('../data/test-dates.txt')

out_path = 'results/' + dataset_path.split('_')[0] + '/'
for date_pred in dates_to_use:

    try:
        test_tf = str_to_datetime(date_pred)
        # test_tf = str_to_datetime2(date_pred)  # formato data diverso
        ...
        # print('Last Train day: {}'.format(datetime_to_str2(train_tf)))  # formato data diverso
        ...
        with open(out_path + 'var_model_norm_{}.csv'.format(date_pred), 'w') as vf:
...
result = {'sq_errors': [], 'real': [], 'predicted': [], 'date': []}
for csv in os.listdir(out_path):
    res = pd.read_csv(out_path + csv)
    error = res['Real'][0] - res['Predicted'][0]
    sq_error = error ** 2
    result['sq_errors'].append(sq_error)
    result['real'].append(res['Real'][0])
